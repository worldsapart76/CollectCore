// Authenticated /pcs/ bulk update: change every selected card's ownership
// status (and optionally notes) in one operation. Server-backed twin of the
// WASM tier's GuestPhotocardBulkAdd — identical UX, but writes go to
// /pcs/copies (pcsData) instead of local SQLite.
//
...
export default function PcsPhotocardBulkAdd({
  selectedCards,
  ownershipStatuses,
  onClose,
  onSaved,
        ...
        try {
          if (realCopies.length === 0) {
            ...
            for (const c of realCopies) {
              ...
              await updatePcsCardCopy(c.copy_id, fields);
            }
            updated++;
          }
        } catch (err) {
