 ...
 *  fetches the body and drives a synthetic click on an object URL. Returns the
 *  number of rows the server actually wrote (cards with no trade copy are
 *  dropped), so the caller can say so. */
export async function exportPhotocardTradesCsv(itemIds) {
  const res = await fetch(`${API}/export/photocard-trades.csv`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ item_ids: itemIds }),
  });
  if (!res.ok) {
    const errorText = await res.text();
    throw new Error(errorText || "Failed to export trade CSV");
  }
  const rows = Number(res.headers.get("X-Exported-Rows"));
  const disposition = res.headers.get("Content-Disposition") || "";
  const filename = disposition.match(/filename=([^;]+)/)?.[1]?.trim() || "photocard_trades.csv";
