...
function ArtistList({ names, onChange }) {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
      {names.map((n, i) => (
        <div key={i} style={{ display: "flex", gap: 4, alignItems: "center" }}>
          <input value={n} onChange={e => update(i, e.target.value)} placeholder="Artist name" style={{ ...inputStyle, flex: 1 }} />
          {names.length > 1 && <button type="button" onClick={() => remove(i)} style={{ ...btnSm, color: "#c62828" }}>✕</button>}
        </div>
      ))}
      <button type="button" onClick={add} style={{ ...btnSm, alignSelf: "flex-start" }}>+ Artist</button>
    </div>
  );
}
      ...
      <div style={{ maxWidth: 680, padding: "16px 0", margin: "0 auto" }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom: 10 }}>
            ...
            <ArtistList names={form.artists} onChange={v => set("artists", v)} />
          </div>
